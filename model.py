...
from label_binarize import my_binarize
from sklearn.utils import shuffle
from read_data import loadGraph
import tqdm
...
def model_architecture(para):
	...
	outputLabel = tf.placeholder(tf.float32, [None, para.pointNumber, para.outputClassN])

	scaledLaplacian = tf.reshape(inputGraph, [-1, para.pointNumber, para.pointNumber])

	weights = tf.placeholder(tf.float32, [None, para.pointNumber])
	lr = tf.placeholder(tf.float32)
	keep_prob_1 = tf.placeholder(tf.float32)
	keep_prob_2 = tf.placeholder(tf.float32)

	features = extract_features(inputPC, tf.constant(True))
	print("The output of feature_extraction is {}".format(features))
	features = tf.reshape(features, [para.batchSize, para.pointNumber, -1])
	print("The output of feature_extraction after reshaping is {}".format(features))

	# gcn layer 1
	gcn_1 = gcnLayer(features, scaledLaplacian, pointNumber=para.pointNumber, inputFeatureN=128,
	...
	print("The output of the first gcn layer is {}".format(gcn_1_output))
	...
	print("The output of the third gcn layer is {}".format(gcn_2_output))
	
	#gcn_layer_4
	#gcn_4 = gcnLayer(gcn_3_output, scaledLaplacian, pointNumber=para.pointNumber, inputFeatureN=para.gcn_4_filter_n,
	#			 outputFeatureN=para.gcn_4_filter_n,
	#		 chebyshev_order=para.chebyshev_2_Order)
	#gcn_4_output = tf.nn.dropout(gcn_4, keep_prob=keep_prob_1)
	#print("The output of the fourth gcn layer is {}".format(gcn_4_output))

	# fully connected layer 1
	fc_layer_1 = fullyConnected(gcn_3_output, inputFeatureN=para.fc_1_in, outputFeatureN=para.fc_1_out)
	...
	fc_layer_2 = fullyConnected(fc_layer_1, inputFeatureN=para.fc_1_out, outputFeatureN=128)
	print("The output of the second fc layer is {}".format(fc_layer_2))


	# =================================Define loss===========================
	predictSoftMax = tf.nn.softmax(fc_layer_2)
	predictLabels = tf.argmax(predictSoftMax, axis=2)
	...
	correct_prediction = tf.equal(predictLabels, tf.argmax(outputLabel, axis=2))
...
def trainOneEpoch(inputCoor, inputLabel, para, sess, trainOperaion, weight_dict, learningRate):
	dataChunkLoss = []
	dataChunkAcc = []
	dataChunkRegLoss = []
	for i in tqdm.tqdm(range(len(inputCoor))):
		inputGraph = loadGraph(inputCoor[i], i,"train")
		xTrain_1, graphTrain_1, labelTrain_1 = inputCoor[i][:,:,:3], inputGraph, inputLabel[i]
		graphTrain_1 = graphTrain_1[i].tocsr()
		labelBinarize = my_binarize(labelTrain_1, [])
...
def evaluateOneEpoch(inputCoor, inputLabel, para, sess, trainOperaion):
	test_loss = []
	test_acc = []
	test_predict = []
	for i in range(len(inputCoor)):
		inputGraph = loadGraph(inputCoor[i], i, "test")
		xTest, graphTest, labelTest = inputCoor[i][:,:,:3], inputGraph, inputLabel[i]
		graphTest = graphTest[i].tocsr()
		labelBinarize = my_binarize(labelTest, [])
